...
func (b *Backend) Destroy(_ context.Context, id resource.ResourceID, observed executor.ObservedResource) error {
	...
	name := strings.TrimPrefix(id.String(), "kind-cluster:")
...
func (b *Backend) configureRegistryMirrors(ctx context.Context, cluster *resource.KindCluster) error {
	if len(cluster.RegistryMirrors) == 0 {
		return nil
	}
	...
	if applyErr := applyLocalRegistryConfigMap(ctx, kube, cluster.RegistryMirrors); applyErr != nil {
		return applyErr
	}
	nodes, err := b.kind.GetNodes(cluster.Name)
	if err != nil {
	...
	for _, mirror := range cluster.RegistryMirrors {
		...
		if err := b.configureRegistryMirror(ctx, nodes, mirror.Host, endpoint, mirror.Upstream); err != nil {
			return err
		}
	}
	return nil
...
func registryEndpointName(ref resource.ResourceRef) (string, error) {
	if container, ok := ref.Resource().(*resource.Container); ok {
		return container.Name, nil
	}
	name, ok := strings.CutPrefix(ref.ID().String(), "container:")
	if !ok || name == "" {
