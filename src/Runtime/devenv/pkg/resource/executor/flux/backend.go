...
type fluxOperations interface {
	Install(components []string, installOpts flux.InstallOptions) error
}
...
func (b *Backend) Apply(
	...
	switch res := r.(type) {
	case *resource.FluxInstallation:
		return executor.NoOutput{}, b.applyFluxInstallation(res)
	default:
...
func (b *Backend) Observe(
	...
	switch res := r.(type) {
	case *resource.FluxInstallation:
		installed, err := b.fluxInstalled(res)
		if err != nil {
			return observed, err
		}
		if !installed {
...
func (b *Backend) applyFluxInstallation(installation *resource.FluxInstallation) error {
	...
	if err := clients.flux.Install(components, flux.LocalInstallOptions()); err != nil {
		return fmt.Errorf("install flux controllers: %w", err)
	}
	for _, component := range components {
		if err := clients.kube.RolloutStatus(component, "flux-system", defaultReadinessTimeout); err != nil {
...
func (b *Backend) fluxInstalled(installation *resource.FluxInstallation) (bool, error) {
	clients, err := b.ensureClusterClients(installation.Cluster)
	if err != nil {
		return false, err
	}
	return clients.kube.Get(kubernetes.NamespaceGVR, "flux-system", "") == nil &&
		clients.kube.Get(kubernetes.DeploymentGVR, "source-controller", "flux-system") == nil, nil
}
